...
// ⭐️⭐️ DODANE: Admin Logs (z katalogu lib)
...
app.use((req, res, next) => {
  const connectSrc = ["'self'", "https://geowidget.easypack24.net", "https://nominatim.openstreetmap.org"].join(" ");
  const csp =
...
function pickToken(req: Request): string | undefined {
  ...
  const cookieToken = c.token || c.jwt || c.auth || c.auth_token || c.access_token || c.id_token || undefined;
...
// ✅✅ DODANE: rejestracja routera logów admina
