...
/* ==================================================================
 *  InPost SDK (easypack24) — single init (minimalny CSS, bez patchy)
 * ================================================================== */
const INPOST_HOST = "https://geowidget.easypack24.net";

function ensureInpostSDK() {
  return new Promise((resolve, reject) => {
    // Minimalny, bezpieczny CSS globalny – tylko to, co MUSI być
    if (!document.getElementById("inpost-zfix")) {
      const st = document.createElement("style");
      st.id = "inpost-zfix";
      st.textContent = `
        .easypack-modal { z-index: 2147483000 !important; position: fixed !important; }
        .easypack-modal .leaflet-container,
        .easypack-modal .leaflet-pane { filter:none !important; mix-blend-mode:normal !important; }
        .easypack-modal, .easypack-widget, .easypack-container, .easypack-widget * { color-scheme: light !important; }
      `;
    ...
    // Sprzątanie duplikatów (np. po HMR)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Poczekaj, aż kontener jest w DOM i ma rozmiar
...
export default function CheckoutPage() {
  ...
  // shippingMethod: standard | locker | point | express | pickup
  const [shippingMethod, setShippingMethod] = useState("standard");
  // courier / locker / point — nazwa przewoźnika
  const [shippingCarrier, setShippingCarrier] = useState("dpd");
  ...
  // Reset wyborów przy zmianie metody/przewoźnika
  useEffect(() => {
    setError("");
    if (shippingCarrier === "inpost" && shippingMethod === "locker") {
      // zostaw wybór z mapy InPost
    } else {
      setLocker({ code: "", name: "" });
    }
    setPickupPoint({ code: "", name: "" });
  ...
  const estimatedDate = useMemo(() => {
    return shippingMethod === "express" ? addBusinessDays(new Date(), 1) : addBusinessDays(new Date(), 2);
  }, [shippingMethod]);
  ...
  const handleOrder = async (e) => {
    ...
    if (shippingMethod === "pickup") {
      ...
      if (shippingCarrier === "inpost") {
        shippingAddr1 = `${shippingAddr1} (Paczkomat InPost: ${locker.code}${
        ...
        const label =
          shippingCarrier === "dhl_box"
            ? "DHL Box"
            : shippingCarrier === "pocztex_automat"
            ? "Pocztex Automat"
            : "Automat";
        shippingAddr1 = `${shippingAddr1} (${label}: ${pickupPoint.code}${
          pickupPoint.name ? `, ${pickupPoint.name}` : ""
        })`;
      ...
      const label =
        shippingCarrier === "orlen"
          ? "ORLEN Paczka"
          : shippingCarrier === "dpd_pickup"
          ? "DPD Pickup"
          : "Pocztex Punkt";
      shippingAddr1 = `${shippingAddr1} (${label}: ${pickupPoint.code}${
        pickupPoint.name ? `, ${pickupPoint.name}` : ""
  ...
  return (
    <>
      ...
      <div className="bg-white/80 dark:bg-white/90 rounded-3xl shadow-xl p-8 max-w-xl mx-auto mt-8 border-2 border-gold">
        <h2 className="text-2xl font-bold text-mainRed mb-6 text-center">Zamówienie</h2>

        {/* Podsumowanie koszyka */}
        <div className="mb-6 rounded-xl border p-4 bg-white space-y-2">
          ...
          {paymentMethod === "cod" && (
            <div className="flex justify-between text-sm text-gray-700">
              <span>Dopłata za płatność</span>
              <span>{fmtAmount(5)} zł</span>
            </div>
          )}
          <div className="flex justify-between mt-2 text-lg font-extrabold text-mainRed">
            <span>Suma</span>
            <span>{fmtAmount(total)} zł</span>
          </div>

          {/* ETA */}
          <div className="text-xs text-gray-600">
            Przewidywana dostawa: <span className="font-semibold">{fmtDate(estimatedDate)}</span>
          </div>
        ...
        <div className="mb-6 rounded-xl border p-4 bg-white">
          <div className="font-bold text-mainRed mb-2">Dane kontaktowe</div>
        ...
        {/* Metoda dostawy */}
        <div className="mb-6 rounded-xl border p-4 bg-white">
          <div className="font-bold text-mainRed mb-3">Metoda dostawy</div>
          ...
          {/* Automaty paczkowe */}
          <div className="space-y-2 text-sm mb-4">
            <div className="font-semibold">Automat paczkowy</div>
                ...
                {shippingCarrier === "inpost" ? (
                  <>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-3 py-2 rounded-lg border-2 border-gold font-semibold text-mainRed hover:bg-gold/20"
                      >
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500">
                      (Wpisz ręcznie kod i nazwę/adres automatu. Mapa jest dostępna tylko dla
                      InPost.)
                    </p>
                  </>
                )}
          ...
          {/* Punkty odbioru */}
          <div className="space-y-2 text-sm mb-4">
            <div className="font-semibold">Punkt odbioru</div>
            {[
              ["orlen", "ORLEN Paczka"],
        ...
        <div className="mb-6 rounded-xl border p-4 bg-white">
          <div className="font-bold text-mainRed mb-2">Adres</div>
          <div className="grid gap-3">
            <input
              className="rounded-lg border p-2"
              type="text"
        ...
        {/* Płatność */}
        <div className="mb-6 rounded-xl border p-4 bg-white">
          <div className="font-bold text-mainRed mb-2">Metoda płatności</div>
          <div className="space-y-2 text-sm">
            <label className="flex items-center gap-2">
              <input
                type="radio"
        ...
        {/* Kod rabatowy */}
        <div className="mb-6 rounded-xl border p-4 bg-white">
          <div className="font-bold text-mainRed mb-2">Kod rabatowy</div>
          <div className="flex gap-2">
            <input
              type="text"
              value={discountCode}
          ...
          {couponMsg && <div className="text-sm text-green-700 mt-2">{couponMsg}</div>}
        </div>

        {/* Faktura */}
        <div className="mb-6 rounded-xl border p-4 bg-white">
          <label className="flex items-center gap-2 font-bold text-mainRed">
            <input
              type="checkbox"
              checked={wantInvoice}
          ...
          {wantInvoice && (
            <div className="mt-3 grid gap-3">
              ...
              />
              <div className="text-xs text-gray-600">* pola wymagane do wystawienia faktury</div>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleOrder} className="flex flex-col gap-4">
          ...
          <Link to="/cart" className="text-center text-mainRed underline hover:text-gold transition">
            ← Wróć do koszyka
          </Link>
        </form>
      </div>
