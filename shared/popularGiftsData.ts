// shared/productsGiftsData.ts
// Możesz też mieć kopię (jeśli chcesz) w: api/src/seed/productsGiftsData.ts lub web/src/data/productsGiftsData.ts
// ale rekomenduję JEDEN plik w /shared + alias @shared/*

// === Typ pojedynczego produktu (wersja “Products”) ===
...
// === Typ wejściowych danych surowych (jak w Twojej tablicy) ===
type RawGift = {
  slug: string;
  name: string;
  description?: string;
...
// === Normalizacja kategorii do slugów ===
...
// === SUROWE DANE (Twoja pełna tablica RawGift[]) ===
...
// === Mapa na docelowy typ (bez undefined w polach opcjonalnych) ===
export const productsGiftsData: PopularGift[] = rawGifts.map((g) => {
  const priceCents =
...
// default export z danymi (wygodny do importu domyślnego)
export default productsGiftsData;

// dodatkowo wystawiamy też rawGifts, jeśli gdzieś chcesz użyć
export { rawGifts };
